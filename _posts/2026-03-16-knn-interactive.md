...
window.KNN = (function() {
  ...
  K.genMoons = function(n) {
    ...
    for (var i = 0; i < half; i++) {
      ...
      pts.push({
        x: Math.cos(t) * 2 + (Math.random() - 0.5) * 1.5,
        y: Math.sin(t) * 2 + (Math.random() - 0.5) * 1.5,
        label: 0
      });
    }
    for (var i = 0; i < n - half; i++) {
      ...
      pts.push({
        x: 1 - Math.cos(t) * 2 + (Math.random() - 0.5) * 1.5,
        y: 0.5 - Math.sin(t) * 2 + (Math.random() - 0.5) * 1.5,
        label: 1
      });
    }
    return pts;
  ...
  K.drawBoundary = function(ctx, pts, W, H, pad, xR, yR, c, k, distFn, weighted, res) {
    ...
    var lowW = Math.ceil(imgW / res);
    var lowH = Math.ceil(imgH / res);
    var imgData = ctx.createImageData(lowW, lowH);
    var c0 = c.class0RGB;
    var c1 = c.class1RGB;
    for (var py = 0; py < lowH; py++) {
      for (var px = 0; px < lowW; px++) {
        var coord = K.fromCanvas(pad + px * res, pad + py * res, W, H, pad, xR, yR);
...
(function() {
  ...
  function drawOne(canvas, weighted, label) {
    ...
    if (pts.length >= 2) {
      ...
      for (var py = 0; py < lowH; py++) {
        for (var px = 0; px < lowW; px++) {
          var coord = KNN.fromCanvas(pad + px * res, pad + py * res, W, H, pad, xR, yR);
...
<tr><td>Manhattan Distance</td><td>Axis-aligned distance (diamond neighborhoods)</td><td>$$\sum \vert x_i - x_i' \vert$$</td></tr>
<tr><td>Chebyshev Distance</td><td>Maximum coordinate difference (square neighborhoods)</td><td>$$\max \vert x_i - x_i' \vert$$</td></tr>
